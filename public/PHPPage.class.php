...
class PHPPage{
	...
	public function limit()
	{
		...
		{
			return  (($p*$limt)-($limt)).','.(($page_max-1)*$limt+($total%$limt));
		}
	...
	public function show(){
		...
		if($p>1){
			$last_page = $p-1;
			echo "<li><a href='$url?{$this->_p}=$last_page{$param}'>上页</a></li>";
			echo $this->_split;
		}
	
		if($p==1){
			echo '<li  class="paginate_button active"><span class="php_page_current">1</span></li>';
		}else{
			echo "<li><a href='$url?{$this->_p}=1{$param}'>1</a></li>";
		}
		echo $this->_split;
		...
		for($i=$start;$i<=$end;$i++){
			if($p == $i){
				echo "<li class='paginate_button active'><span class='php_page_current'>".$i.'</span></li>';
			}else{
				echo "<li><a href='$url?{$this->_p}={$i}{$param}'>".$i.'</a></li>';
			}
			echo $this->_split;
		}
		if($end<$page_max-1){
		...
		if($page_max>1){
			if($p==$page_max){
				echo "<li class='paginate_button active'><span class='php_page_current'>$page_max</span></li>";
			}else{
				echo "<li><a href='$url?{$this->_p}={$page_max}{$param}'>$page_max</a></li>";
			}
			echo $this->_split;
		}
		...
		if($p<$page_max){
			$next_page = $p+1;
			echo "<li><a href='$url?{$this->_p}=$next_page{$param}'>下页</a></li>";
			echo $this->_split;
		}
		
	/*	echo '<span class="php_page_info">';
		echo '一共'.$this->_total.' 条数据,当前第 '.$p.' 页,共 '.$page_max.' 页';
		echo '</span>';*/
